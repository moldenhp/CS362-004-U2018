...
int use_adventurer(int drawntreasure, struct gameState *state, int currentPlayer, int cardDrawn, int temphand[], int z)
{
...
int use_smithy(int i, int currentPlayer, struct gameState *state, int handPos)
{
  //+3 Cards
  // for(i = 0; i < 3; i++)
  for (i = 0; i < 2; i++) // bug
...
int use_village(int currentPlayer, struct gameState *state, int handPos)
{
  //+1 Card
  drawCard(currentPlayer, state);
...
int use_great_hall(int currentPlayer, struct gameState *state, int handPos)
{
  //+1 Card
  drawCard(currentPlayer, state);
...
int use_embargo(struct gameState *state, int handPos, int currentPlayer, int choice1)
{
  //+2 Coins
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  case adventurer:
    return use_adventurer(drawntreasure, state, currentPlayer, cardDrawn, temphand, z);
  /*
      while(drawntreasure<2){
  ...
  case smithy:
    return use_smithy(i, currentPlayer, state, handPos);
    /*
      //+3 Cards
      for (i = 0; i < 3; i++)
	{
  ...
  case village:
    return use_village(currentPlayer, state, handPos);
  /*
      //+1 Card
      drawCard(currentPlayer, state);
  ...
  case great_hall:
    return use_great_hall(currentPlayer, state, handPos);
  /*
      //+1 Card
      drawCard(currentPlayer, state);
  ...
  case embargo:
    return use_embargo(state, handPos, currentPlayer, choice1);
  /*
      //+2 Coins
      state->coins = state->coins + 2;
